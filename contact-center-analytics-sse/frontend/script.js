document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSopTable(data) {
        if (data.loading) {
            sopViewContainer.innerHTML = `<p class="text-muted">Loading...</p>`;
            return;
        }
        if (data.error || !data.sop_checklist) {
            sopViewContainer.innerHTML = `<div class="alert alert-danger"><strong>Error:</strong> Could not load Script Adherence data.</div>`;
            return;
        }
        let tableHtml = `
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="table-light-custom-header">
                            <tr><th>Key</th><th>Compliance Point</th></tr>
                        </thead>
                        <tbody>
                            ${data.sop_checklist.map(item => `
                                <tr>
                                    <td><span class="badge bg-secondary">${item.key || 'N/A'}</span></td>
                                    <td>${item.point || 'N/A'}</td>
                                </tr>`).join('')}
                        </tbody>
                    </table>
                </div>
    ...
    function renderQuestionnaireTable(data) {
        if (data.loading) {
            questionnaireViewContainer.innerHTML = `<p class="text-muted">Loading...</p>`;
            return;
        }
        if (data.error || !data.lead_scoring_questionnaire) {
            questionnaireViewContainer.innerHTML = `<div class="alert alert-danger"><strong>Error:</strong> Could not load Questionnaire data.</div>`;
            return;
        }
        let tableHtml = `
            <div class="card shadow-sm">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="table-light-custom-header">
                            <tr><th>Key</th><th>Question</th><th>Priority Answers</th></tr>
                        </thead>
                        <tbody>
                            ${data.lead_scoring_questionnaire.map(item => `
                                <tr>
                                    <td><span class="badge bg-secondary">${item.key || 'N/A'}</span></td>
                                    <td>${item.question || 'N/A'}</td>
                                    <td>${(item.priority_answers || []).map(ans => `<span class="badge bg-success me-1">${ans}</span>`).join(' ')}</td>
                                </tr>`).join('')}
                        </tbody>
                    </table>
                </div>
    ...
    function displayResults(data) {
        if (!data) {
            alert("Received empty analysis results from the server.");
            return;
        }
        currentAnalysisResult = data;
        nav.ask.classList.remove('disabled');

        renderLeadScore(data.call_quality_summary);
        renderPersona(data.customer_persona);
    ...
    function renderLeadScore(summary) {
        ...
        container.innerHTML = `
            ...
            <p class="text-muted-custom small mt-1">${summary.justification || ''}</p>
        `;
    }
    ...
    function renderPersona(persona) {
        ...
        if (!persona) {
            container.innerHTML = `<p class="text-muted">No persona data available</p>`;
            return;
        }
        container.innerHTML = `
            <dl class="row mb-0">
                <dt class="col-sm-6 text-primary-gradient">Sentiment</dt>
                <dd class="col-sm-6">${persona.sentiment || 'N/A'}</dd>
                <dt class="col-sm-6 text-primary-gradient">Price Sensitivity</dt>
                <dd class="col-sm-6">${persona.price_sensitivity?.rating !== undefined ? persona.price_sensitivity.rating + ' / 5' : 'N/A'}</dd>
                <dt class="col-sm-6 text-primary-gradient">Quality Sensitivity</dt>
                <dd class="col-sm-6">${persona.quality_sensitivity?.rating !== undefined ? persona.quality_sensitivity.rating + ' / 5' : 'N/A'}</dd>
                <dt class="col-sm-6 text-primary-gradient">Technical Proficiency</dt>
                <dd class="col-sm-6">${persona.technical_proficiency?.rating !== undefined ? persona.technical_proficiency.rating + ' / 5' : 'N/A'}</dd>
                <dt class="col-sm-6 text-primary-gradient">Inferred Occupation</dt>
                <dd class="col-sm-6">${persona.occupation_inference || 'N/A'}</dd>
                <dt class="col-sm-6 text-primary-gradient">Lead Source</dt>
                <dd class="col-sm-6">${persona.lead_source || 'N/A'}</dd>
            </dl>
        `;
    }
    ...
    function renderCallQualityCallouts(summary) {
        ...
        container.innerHTML = `
            <div class="col-md-3"><div class="callout-card"><h6>Overall Call Quality</h6><p>${summary.overall_call_quality || 'N/A'}</p></div></div>
            <div class="col-md-3"><div class="callout-card"><h6>Overall Lead Quality</h6><p>${summary.overall_lead_quality || 'N/A'}</p></div></div>
            <div class="col-md-6"><div class="callout-card"><h6>Required Follow-up</h6><p>${summary.required_follow_up || 'None specified'}</p></div></div>
            <div class="col-md-3"><div class="callout-card"><h6>Greetings Check</h6><p>Performed ${checkIcon(summary.greetings_check)}</p></div></div>
            <div class="col-md-3"><div class="callout-card"><h6>Rapport Building</h6><p>Attempted ${checkIcon(summary.rapport_building_check)}</p></div></div>
            <div class="col-md-3"><div class="callout-card"><h6>Needs Analysis</h6><p>Performed ${checkIcon(summary.needs_analysis_check)}</p></div></div>
            <div class="col-md-3"><div class="callout-card"><h6>Product Recommendation</h6><p>Performed ${checkIcon(summary.product_recommendation_check)}</p></div></div>
        `;
    }
    ...
    function renderSopAdherence(report) {
        ...
        if (!report || !Array.isArray(report) || report.length === 0) {
            container.innerHTML = `<p class="text-muted">No Script Adherence report available.</p>`;
            return;
        }
        ...
        const getStatusIcon = (compliance) => {
            if (compliance === "Fully Covered") return '✅';
            if (compliance === "Partially Covered") return '⚠️';
            if (compliance === "Not Covered") return '❌';
            return '❓';
        };
        let tableHtml = `
            <h5 class="mb-3">Overall Score: <span class="text-success-custom">${compliedCount}</span>/<span class="text-primary-main-text">${totalCount}</span> (<span class="text-warning-custom">${percentage}%</span>)</h5>
            <div class="table-responsive">
                <table class="table table-sm">
                    <thead><tr><th class="text-primary-gradient">Status</th><th class="text-primary-gradient">Script Point</th><th class="text-primary-gradient">Commentary</th><th class="text-primary-gradient">Evidence</th></tr></thead>
                    <tbody>
                        ${report.map(item => `
                            <tr>
                                <td class="text-center">${getStatusIcon(item.compliance)}</td>
                                <td>${item.sop_point}</td>
                                <td><em class="text-muted-custom small">${item.commentary || 'N/A'}</em></td>
                                <td><em class="text-muted-custom small">${item.evidence || 'Not found'}</em></td>
                            </tr>`).join('')}
                    </tbody>
                </table>
            </div>`;
    ...
    function renderTranscript(transcript, containerId) {
        ...
        if (!transcript || transcript.length === 0) {
            container.innerHTML = `<p class="text-muted">Transcript not available</p>`;
            return;
        }
        container.innerHTML = transcript.map(line => {
            const speakerClass = line.speaker?.toLowerCase().includes('agent') ? 'speaker-agent' : 'speaker-customer';
            return `<div class="transcript-line"><span class="${speakerClass}">${line.speaker || 'Unknown'}:</span> <span>${line.transcript_english}</span></div>`
        }).join('');
    }
    ...
    function setupAskQuestionsPage() {
        ...
        if (!currentAnalysisResult) {
            container.innerHTML = `<div class="alert alert-warning">Please process a call on the Home page first to enable this feature.</div>`;
            return;
        }
        container.innerHTML = `
            <div id="chat-container"></div>
            <form id="ask-question-form" class="mt-3">
                <div class="input-group">
                    <input type="text" class="form-control" id="question-input" placeholder="Ask a question about the call..." required>
                    <button type="submit" class="btn btn-primary-gradient">Send</button>
                </div>
            </form>
            <div class="accordion mt-4" id="transcriptAccordion">
                    ...
                    <div id="collapseOne" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#transcriptAccordion">
                        <div class="accordion-body">
                            <div class="transcript-grid">
                                <div class="transcript-column card shadow-sm">
                                    <div class="card-header bg-gradient-header">Original Language</div>
                                    <div class="card-body transcript-box" id="transcript-original-col"></div>
                                </div>
                                <div class="transcript-column card shadow-sm">
                                    <div class="card-header bg-gradient-header">English Translation</div>
                                    <div class="card-body transcript-box" id="transcript-english-col"></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    function renderDualTranscript(transcript) {
        ...
        const render = (segment, key) => `
            <div class="transcript-line">
                <span class="${segment.speaker?.toLowerCase().includes('agent') ? 'speaker-agent' : 'speaker-customer'}">
                    ${segment.speaker || 'Unknown'}:
                </span>
                <span>${segment[key] || '(Not available)'}</span>
            </div>`;
